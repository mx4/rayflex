...
- **Rays that miss everything return black** — there is no sky. Enclose the scene with geometry, or accept a black void. ⚠️ **A giant emissive "sky dome" sphere does NOT work for ambient fill under NEE** — it lights nothing (see Known Bugs). It's still *visible* as a background to camera/specular rays, so it works as a backdrop, but contributes zero diffuse illumination. For fill, use a large dim emissive sphere placed *outside* the scene (not enclosing it).
...
- **An enclosing emissive sphere (sky dome) contributes zero light under NEE.** Verified empirically: a dome-only scene at `ke=0.5` renders pure black `[0,0,0]`. Both paths to it are cut. (1) NEE registers it as a light (it's a sphere with `ke`), samples a point on it, and computes `cos_l = n_l·(-wi)`; the sampled normal points *outward* while the shaded surface is *inside*, so `cos_l < 0` and `direct_light`'s one-sided check rejects it. (2) The diffuse continuation ray *does* hit the dome, but its emission is suppressed exactly because it is an NEE light (the anti-double-counting rule in `trace_ray_path`). Net: no diffuse fill. The dome is still visible to camera/specular rays (those pass `count_emission = true`), so it works as a *backdrop* but not as a *light*. This regressed the previously-documented sky-dome trick when NEE landed. Fix options: handle the point-inside-sphere case in `direct_light` (flip the sampled normal when the shading point is inside), or skip NEE registration for spheres that enclose the scene so the continuation path picks their emission back up. Blocks the "chrome object needs an environment to reflect" recipe.
