...
- **⚠️ A `triangle.N` light wound the wrong way DESTROYS its direct lighting — it is not merely a noise issue.** Both paths to the panel are cut: (1) NEE — `direct_light` gates on `cos_l = light_normal·(-wi) > 0`, so every shadow-ray sample toward a backwards panel is rejected; (2) BSDF — a *diffuse* continuation ray that lands on it returns **zero**, because emission is suppressed for anything registered as an NEE light (the anti-double-counting rule in `trace_ray_path`). Net: **no diffuse surface ever receives direct light from that lamp.** The one path that still works is a **specular** bounce, which passes `count_emission = true` — so a mirror-rich room stays plausibly lit (via light routed through its mirrors) and hides the bug, while a diffuse-only room goes near-black. Measured on `gold-gallery` (winding-only A/B at equal spp): fixing it gave **+73% mean brightness and −44% noise**. `gold-gallery`, `suzanne-bust` and `torus-knot` all shipped with this bug — all three were built from the same triangle-construction snippet. To check by hand: for triangle `(p0,p1,p2)`, compute `normal = (p1-p0) × (p2-p0)`; it must point from the light *into* the room. Fix by swapping the last two vertices: `(p00,p10,p11)` → `(p00,p11,p10)`. Nothing warns you — always verify by computing the normal, never by eye (an emitter looks correct from both sides, since the *camera* ray also passes `count_emission = true`).
